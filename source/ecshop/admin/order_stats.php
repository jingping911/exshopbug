...
if ($_REQUEST['act'] == 'list')
{
    ...
    else
    {
        ...
        $smarty->assign('result',json_encode($order_info));
        ...
        $res= $db->getAll($sql);
        foreach ($res as $k => $v){
            $res[$k]['value'] = $v['order_num'];
            $res[$k]['name'] = $v['pay_name'];
            unset($res[$k]['order_num']);
            unset($res[$k]['pay_name']);
            unset($res[$k]['pay_id']);
            $res[$k] = (object)$res[$k];
        }
        $smarty->assign('res',json_encode($res));
        ...
        $sql = 'SELECT sp.shipping_id, sp.shipping_name AS ship_name, COUNT(i.order_id) AS order_num ' .
               ...
               "GROUP BY i.shipping_id ORDER BY order_num DESC";
        $ship_res = $db->query($sql);
        $res_data= $db->getAll($sql);
        foreach ($res_data as $k => $v){
            $res_data[$k]['value'] = $v['order_num'];
            $res_data[$k]['name'] = $v['ship_name'];
            unset($res_data[$k]['order_num']);
            unset($res_data[$k]['ship_name']);
            unset($res_data[$k]['shipping_id']);
            $res_data[$k] = (object)$res_data[$k];
        }
        $smarty->assign('res_data',json_encode($res_data));
