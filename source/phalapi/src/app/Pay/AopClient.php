...
class AopClient
{
	...
	*/
	...
	function verify($data, $sign, $rsaPublicKeyFilePath, $signType = 'RSA')
	{
		...
		//            wordwrap(self::$zh_public_key, 64, "\n", true) .
		//            "\n-----END PUBLIC KEY-----";
		//
		//        $key = openssl_get_publickey($publicKey);
		//        $ok = openssl_verify($data,base64_decode($sign), $key, 'SHA256');
		//        openssl_free_key($key);
		//	    var_dump($sign);die;
		//	    var_dump($this->alipayrsaPublicKey);die;
		...
		if ("RSA2" == $signType) {
			$result = openssl_verify(trim($data), base64_decode(trim($sign)), $res, "SHA256");
		} else {
