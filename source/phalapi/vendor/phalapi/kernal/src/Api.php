...
```
    ...
    protected function userCheck() {
        ...
        if (defined('APP_KEY') && defined('APP_SECRET')) {
            if (APP_KEY == $app_key) {
                $mapi_secret = APP_SECRET;
            }
        }
        if (empty($app_key) || empty($mapi_secret)) { // 如果 app_key 或 对应的 app_secret不存在 
            throw new BadRequestException('no app_key or app_secret', 1);
        }
        $sign = 'ANDYJJ' . $this->assemble($params) . $mapi_secret;
